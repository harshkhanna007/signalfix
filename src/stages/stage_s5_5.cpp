...
//      brief blip at t=7   → CRITICAL held (hysteresis: need 4.5 s clear to
//      drop)
//
...
DriftLevel StageS55DriftPersistence::classify_with_hysteresis(
    float persistence_s, DriftLevel current_level) noexcept {
  // ── Attempt upgrade (current → higher) ───────────────────────────────────
  if (persistence_s >= T_CRITICAL_UP)
    return DriftLevel::CRITICAL;
  if (persistence_s >= T_CONFIRMED_UP)
    return DriftLevel::CONFIRMED;
  if (persistence_s >= T_BUILDUP_UP)
    return DriftLevel::BUILDUP;
...
float StageS55DriftPersistence::compute_confidence(
    float persistence_s, float drift_gsigma, float roc_severity) noexcept {
  // Normalize each factor to [0, 1].
  const float f_persist = std::min(1.0f, persistence_s / T_CRITICAL_UP);

  const float f_gsigma =
      (CONF_GSIGMA_CAP > 0.0f)
  ...
  const float confidence =
      W_PERSISTENCE * f_persist + W_GSIGMA * f_gsigma + W_ROC * f_roc;
...
// ---------------------------------------------------------------------------
StageResult
StageS55DriftPersistence::process(MeasurementEnvelope &envelope,
  ...
  //   Tier 2 (weak):  gsigma or ROC above weak thresh   → weight 0.4 (+0.15
  //   combo) Tier 3 (sub):   gsigma or ROC above sub thresh    → weight 0.15
  ...
  if (drift_active) {
    ...
    if (channel_state.drift_clear_time_s >= RECOVERY_GATE_S) {
      ...
      const float gsigma_safe =
          std::max(0.0f, static_cast<float>(channel_state.drift_gsigma));
      const float decay_rate = BASE_DECAY_RATE / (1.0f + gsigma_safe);

      channel_state.drift_persistence_time_s = std::max(
          0.0f, channel_state.drift_persistence_time_s - decay_rate * dt_s);
    }
  ...
  const DriftLevel new_level = classify_with_hysteresis(
      channel_state.drift_persistence_time_s, channel_state.drift_level);

  channel_state.drift_level = new_level;

  // 3b. Confidence — computed regardless of level so it's always readable.
  const float confidence =
      compute_confidence(channel_state.drift_persistence_time_s,
                         channel_state.drift_gsigma, roc_severity);

  channel_state.drift_confidence = confidence;

  // ── Continuous Severity Metric (drift_score) ─────────────────────────────
  // 1. Evidence-Based Accumulation
  const float norm_persist =
      std::min(1.0f, std::max(0.0f, channel_state.drift_persistence_time_s /
                                        T_CRITICAL_UP));
  const float norm_gsigma =
      (CONF_GSIGMA_CAP > 0.0f)
          ? std::min(1.0f, std::max(0.0f, static_cast<float>(
                                              channel_state.drift_gsigma) /
                                              CONF_GSIGMA_CAP))
          : 0.0f;
  const float norm_roc = std::isfinite(roc_severity)
                             ? std::min(1.0f, std::max(0.0f, roc_severity))
                             : 0.0f;
  ...
  if (signal_weight > 0.0f) {
    channel_state.drift_score +=
        evidence * level_multiplier * ACCUM_RATE * dt_s;
  } else {
    // 4. Intelligent Decay
    if (channel_state.drift_clear_time_s >= RECOVERY_GATE_S) {
      const float gsigma_safe =
  ...
  channel_state.drift_score =
      std::max(0.0f, std::min(100.0f, channel_state.drift_score));
  envelope.drift_score = channel_state.drift_score; // Export for telemetry

  // ==========================================================================
  // STATE CONSISTENCY — S5.5 does NOT modify envelope.status or failure_hint.
  // The single source of truth is S5 (stage_s5_rate_of_change.cpp):
  //   channel_state.drift_active → DRIFT_EXCEEDED flag → failure_hint = DRIFT
  //
  // S5.5 is an evidence accumulator only. It tracks DriftLevel and drift_score
  // for continuity/confidence metrics, but NEVER overrides pipeline state.
  // ==========================================================================
