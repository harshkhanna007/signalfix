...
void log_configuration_audit(const RocDetectionConfig &cfg) noexcept {
  ...
  std::printf("  %-30s %.3f\n", "alpha (learning rate):",
              static_cast<double>(cfg.alpha_learning));
  std::printf("  %-30s %.3f\n",
              "beta_up (rise rate):", static_cast<double>(cfg.beta_up));
  std::printf("  %-30s %.3f\n",
              "beta_down (fall rate):", static_cast<double>(cfg.beta_down));
  std::printf("  %-30s %.4f\n", "beta_baseline (anchor):",
              static_cast<double>(cfg.beta_baseline));
  std::printf("  %-30s %.2f / %.2f\n",
              "sigma_governor_ratios:", cfg.sigma_min_ratio,
              cfg.sigma_max_ratio);
...
// ---------------------------------------------------------------------------
static void stage_s0_adapter(signalfix::MeasurementEnvelope &env,
                             signalfix::ChannelState &, double raw) noexcept {
  env.raw_value = raw;
  env.calibrated_value = raw;
}
static void stage_s0_watchdog(signalfix::MeasurementEnvelope &env,
                              signalfix::ChannelState &,
...
static void stage_s3_plausibility(signalfix::MeasurementEnvelope &env,
                                  signalfix::ChannelState &) noexcept {
  if (env.calibrated_value > 1000000.0)
    env.status |= signalfix::SampleStatus::HARD_INVALID;
}
static void stage_s6_packager(signalfix::MeasurementEnvelope &env,
                              signalfix::ChannelState &) noexcept {
...
const char *describe_classification(signalfix::Classification c) noexcept {
  ...
  // ACTIVE_FAILURE: pipeline confirmed a real signal anomaly (ROC, drift,
  // etc.). Previously "REAL_EVENT" — renamed to be immediately actionable on
  // sight.
  case C::REAL_EVENT:
    return "ACTIVE_FAULT  ";
...
static constexpr int SENSOR_MAP_SIZE =
    static_cast<int>(sizeof(SENSOR_MAP) / sizeof(SENSOR_MAP[0]));

[[nodiscard]] inline const char *get_sensor_name(uint32_t channel_id) noexcept {
  for (int i = 0; i < SENSOR_MAP_SIZE; ++i) {
    if (SENSOR_MAP[i].channel_id == channel_id)
      return SENSOR_MAP[i].sensor_name;
  }
  return "Unknown Sensor";
}

[[nodiscard]] inline int get_sensor_column(uint32_t channel_id) noexcept {
  for (int i = 0; i < SENSOR_MAP_SIZE; ++i) {
    if (SENSOR_MAP[i].channel_id == channel_id)
      return SENSOR_MAP[i].dataset_column_index;
  }
  return 2; // safe fallback to T30
}
const char *describe_status(signalfix::SampleStatus s) noexcept {
  using S = signalfix::SampleStatus;
  if (signalfix::has_flag(s, S::STALE))
    return "STALE       ";
  if (signalfix::has_flag(s, S::HARD_INVALID))
    return "HARD_INVALID";
  if (signalfix::has_flag(s, S::MISSING))
    return "MISSING     ";
  if (signalfix::has_flag(s, S::ROC_EXCEEDED))
    return "ROC_EXCEEDED";
  if (signalfix::has_flag(s, S::DRIFT_EXCEEDED))
    return "DRIFT_EXCEEDED";
  return "NOMINAL     ";
}

void log_sample_full(int n, const signalfix::MeasurementEnvelope &env,
                     const signalfix::ChannelState &ch,
                     const ProcessingAnnotation &ann) noexcept {
  char raw_buf[14], cal_buf[14], rr_buf[10], rc_buf[10], mu_buf[10],
      sig_buf[10], lim_buf[10], base_buf[10];
  if (std::isnan(env.raw_value))
    std::snprintf(raw_buf, 14, "%10s", "NaN");
  else
    std::snprintf(raw_buf, 14, "%10.1f", env.raw_value);
  ...
  std::snprintf(sig_buf, 10, "%7.1f",
                (double)std::sqrt(std::max(0.0, ch.roc_m2)));
  ...
  std::printf("  [%3d] rr=%s | μ=%s σ=%s streak=%-2u | T=%s Status=%-12s "
              "Hint=%-5s(%.2f) dur=%-2u | Class=%s\n",
              n, rr_buf, mu_buf, sig_buf, ch.roc_normal_streak, lim_buf,
              describe_status(env.status), f_hint,
              (double)env.failure_confidence, env.failure_duration,
...
static bool noop_processed_output(const signalfix::ProcessedSample &,
                                  void *) noexcept {
  return true; // Always succeed, no side effects
}

void run_stream_core(const char *name, const RawSample *samples, int n,
                     signalfix::ChannelState &ch) noexcept {
  // [Rev 2.8.4 0.1% Tier Certified Parameters]
  // Removed local overrides to use centralized tuning.
  ...
  for (int i = 0; i < n; ++i) {
    ...
    env.arrival_time_us = time;
    env.delta_t_us = ch.nominal_delta_t_us;
    ...
    if (std::isnan(raw)) {
      ...
      log_sample_full(i, env, ch,
                      {signalfix::derive_classification(env), 0.0f});
...
static uint32_t lcg_next(uint32_t &s) noexcept {
  s = s * 1'664'525u + 1'013'904'223u;
  return s;
}

void run_stream_scenario(const char *name, int samples_n, double baseline,
                         double noise, int spike_at,
                         double spike_val) noexcept {
  RawSample sbuf[128];
  uint32_t rng = 0x1234;
  ...
  for (int i = 0; i < samples_n && i < 128; ++i) {
    ...
    double n =
        (static_cast<double>(lcg_next(rng)) / 4294967295.0 * 2.0 - 1.0) * noise;
