...
#include "include/signalfix/module1/stages/s7.hpp"
#include "include/signalfix/module1/types.hpp"
#include "include/signalfix/module1/channel_state.hpp"
    ...
    ChannelState&        channel_state) noexcept
{
    ...
    // ── Failure Persistence & Hysteresis (Rev 2.7 Stateful) ──────────────────
    ...
    // Step 1: Hard Failure Override (Safety Critical)
    // NaN, Inf, or suggested INVALID immediately latch mode with zero hysteresis.
    if (suggested == FailureMode::INVALID || std::isnan(envelope.calibrated_value))
    {
    ...
    // CASE A: Failure detected (suggested != NONE)
    else if (suggested != FailureMode::NONE)
    {
        if (suggested == fs.latched_hint)
        {
            // Same failure mode continues: increment duration and reset recovery counter
            if (fs.duration_frames < 0xFFFFu) fs.duration_frames++;
            fs.exit_counter = 0u;
             
            // Confidence smoothing: latched = 0.7 * old + 0.3 * new
            fs.latched_confidence = (0.7f * fs.latched_confidence) + (kAlpha * suggested_conf);
        }
        else
        {
            // Possible new failure mode or transition from NONE
            fs.enter_counter++;
            if (fs.enter_counter >= kEnterThreshold)
            {
                // Threshold met: switch to new failure mode
                fs.latched_hint       = suggested;
                fs.duration_frames    = 1u;
                fs.exit_counter       = 0u;
                fs.enter_counter      = 0u;
                fs.latched_confidence = suggested_conf; // Initialise confidence
                fs.last_failure_ts_us = envelope.arrival_time_us;
            }
        }
    }
    // CASE B: Recovery or Nominal stream (suggested == NONE)
    else
    {
        if (fs.latched_hint != FailureMode::NONE)
        {
            ...
            {
                // Recovery confirmed: return to baseline NOMINAL state
            ...
            {
                // Still within exit window: keep failure active
                if (fs.duration_frames < 0xFFFFu) fs.duration_frames++;
        ...
        {
            // Already nominal: reset all tracking context
            fs.duration_frames    = 0u;
            fs.latched_confidence = 0.0f;
            fs.enter_counter      = 0u;
            fs.exit_counter       = 0u;
